...
void AudioHelper::UnloadEvon()
{
	...
	LogUtil::printInfo( "Now OpenAL environment unloaded.\n");
	isEvonInit = false;
}
...
XCWavFile AudioHelper::loadWavFromFile(const std::string& filename)
{
	...
	if (wavIter != wavSourceGroup.end()) {
		...
		if (success) {
			...
			LogUtil::printInfo("Load wav from file " + filename + "\n");
		}
		else {
			LogUtil::printError("Failed to load wav from file " + filename + "\n");
		}
	}
	return wavFile;
}
...
ALuint AudioHelper::loadWavByAlut(const std::string & filename)
{
	...
	if (!alIsBuffer(buffer)) {
		LogUtil::printError("Failed to load audio file " + filename + '\n');
	}
	else {
		LogUtil::printInfo("Load audio from file " + filename + '\n');
	}
	return  buffer;
}
...
bool AudioHelper::loadWavFile(const std::string filename, XCWavFile *wavFile) {
	...
	try {
		...
		LogUtil::printError(error + " : trying to load " + filename + '\n');
		if (soundFile != NULL)
			fclose(soundFile);
		return false;
	}
